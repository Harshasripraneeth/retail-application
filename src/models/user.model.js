...
User.getAllData = async function (req, result) {
  ...
  const sqlQuery = 'WITH R1 AS (SELECT TOP 10000 * FROM TRANSACTIONS1 ORDER BY HSHD_NUM ASC), R2 AS (SELECT HSHD_NUM, BASKET_NUM, PURCHASE_DATE, PRODUCTS1.PRODUCT_NUM, DEPARTMENT, COMMODITY, SPEND, UNITS, STORE_REGION, WEEK_NUM,YEAR FROM R1 INNER JOIN PRODUCTS1 ON R1.PRODUCT_NUM = PRODUCTS1.PRODUCT_NUM) SELECT HOUSEHOLDS1.HSHD_NUM, BASKET_NUM, PURCHASE_DATE, PRODUCT_NUM, DEPARTMENT, COMMODITY, SPEND, UNITS, STORE_REGION, WEEK_NUM,YEAR, LOYALTY_FLAG, AGE_RANGE, MARITAL_STATUS, INCOME_RANGE, HOMEOWNER_DESC, HSHD_COMPOSITION, HH_SIZE, CHILDREN FROM R2 INNER JOIN HOUSEHOLDS1 ON HOUSEHOLDS1.HSHD_NUM = R2.HSHD_NUM';    
  try {
        const pool = await poolPromise;
        const data = await pool.request()
        .query(sqlQuery);
